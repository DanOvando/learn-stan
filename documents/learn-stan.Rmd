...
data <- list(n = nrow(mack_data),
               r = mack_data$r,
             ssb = mack_data$ssb,
               max_r = max(mack_data$r)
             )

mack_fit <- stan(file = here::here("scripts","bh_model.stan"),
                 data = data,
                 chains = 4,
                 warmup = warmups,
...
rstanarm::launch_shinystan(mack_fit)
...
rstan::check_divergences(mack_fit)
...
mack_diagnostics <- rstan::get_sampler_params(mack_fit) %>% 
  set_names(1:4) %>% 
...
```{r}
 mack_summary <- summary(mack_fit)$summary %>% 
  as.data.frame() %>% 
...
mack_pars <- mack_mcmc[ c('h','alpha','sigma')] %>% 
...
pp_rhat <- mack_summary %>% 
  filter(str_detect(variable,'pp_rhat')) %>% 
  mutate(ssb = mack_data$ssb)
