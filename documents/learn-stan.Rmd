...
$$ \frac{0.8 \times \alpha \times h \times SSB}{0.2 \times \alpha(1 - h) + (h - 0.2) \times SSB}$$
...
Now that we know what we need to model, we just have to code it. We'll work step by step through this process now over in "src/bh_model.stan". 
...
ricker_fit <- stan(file = here::here("src","generic_stock_recruit.stan"),
                 data = data,
                 chains = 4,
                 warmup = warmups,
                 iter = total_iterations,
...
hake_fit <- stan(
  file = here::here("src","schaefer.stan"),
  data = hake_data,
  chains = n_chains,
  warmup = warmups,
  iter = total_iterations,
...
ss_hake_fit <- stan(
  file = here::here("src","ss_schaefer.stan"),
  data = hake_data,
  chains = n_chains,
  warmup = warmups,
  iter = total_iterations,
